...
# Linear regression
Linear regression aims to explain the relationship between a single continuous variable and one (or more) explanatory variable(s) using a linear equation:

<!-- [$Y = \beta_0 + \beta_1X_1 + \beta_2X_2 + \dots + \beta_kX_k$]{.center} -->

<!-- In this equation, $Y$ is the outcome being modelled, $X_i$ are explanatory variables, and $\beta_i$ are known as regression **coefficients**.  -->
...
lm_flipper <- lm(body_mass_g ~ flipper_length_mm, # <1>
                 data = penguins)
...
- `(Intercept)`: this returns the estimate of $\beta_0$ from @eq-linearreg, the expected value of the outcome where all covariates are 0. 

:::{.callout-note}
Often, the intercept value will not have a meaningful interpretation. For example, here it tells us the expected body mass of penguins with flipper length 0mm is [`{r} round(lm_flipper$coefficients[1], 2)`g]{.highlight}. 
:::
...
The 95% confidence interval for the flipper length coefficient it [`{r} round(confint(lm_flipper)[2, 1], 2)`, `{r} round(confint(lm_flipper)[2, 2], 2)`]. Therefore, we are 95% confident that for every millimeter longer a penguin's flipper is, we would expect their body mass to be between `{r} round(confint(lm_flipper)[2, 1], 2)` and `{r} round(confint(lm_flipper)[2, 2], 2)` higher.
...
[body mass = `{r} round(coef(lm_flipper_spec)[1], 2)` + `{r} round(coef(lm_flipper_spec)[2], 2)` $\times$ flipper length + `{r} round(coef(lm_flipper_spec)[3], 2)` $\times$ chinstrap + `{r} round(coef(lm_flipper_spec)[4], 2)` $\times$ gentoo]{.highlight .centre}
...
## Model comparisons
When choosing the best possible model to address a research question, the aim is usually to find the most [parsimonious]{.highlight} model for the job. That means the simplest model to explain as much as possible. Model choice should first and foremost be driven by the motivation for the analysis (the research question), our prior knowledge of what other factors are important, and common sense based on preliminary checks. 

[Model choice should not be determined solely by p-values]{.highlight}. 
