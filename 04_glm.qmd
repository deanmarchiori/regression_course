...
pacman::p_load(tidyverse, palmerpenguins, Metrics, car)
...
GLM generalises the linear model framework by fitting the linear model to some transformation of the outcome. This transformation is known as the [link function]{.highlight}. Each regression type has an associated link function, [$g(Y)$]{.highlight}, making the updated model formula:
...
cancer_reg <- read_csv("data/cancer_reg.csv") # <1>
...
:::{.highlight .centre}
$ln$(number of deaths) = `{r} round(coef(glm_cancer_pov)[1], 2)` `{r} round(coef(glm_cancer_pov)[2], 2)` $\times$ poverty
:::
...
number of deaths = exp(`{r} round(coef(glm_cancer_pov)[1], 2)` `{r} round(coef(glm_cancer_pov)[2], 2)` $\times$ poverty)

= exp(`{r} round(coef(glm_cancer_pov)[1], 2)`) $\times$ exp(`{r} round(coef(glm_cancer_pov)[2], 2)` $\times$ poverty)
...
For every 1 percentage point increase in poverty, the number of deaths is expected to decrease by `{r} (1 - round(exp(coef(glm_cancer_pov)[2]), 4)) * 100` %. To find the expected difference in counties where the poverty level was 10% higher, we multiple the transformed coefficient by itself 10 times and convert it into a percentage change:
...
Therefore, we are 95% confident that the relative number of deaths in a county with a poverty level 1 percentage point higher will be between `{r} round(exp(confint(glm_cancer_pov))[2, 1], 2)` and `{r} round(exp(confint(glm_cancer_pov))[2, 2], 2)`.
...
To avoid masking the relationship we are interested in modelling, we can introduce the population into the model. This can be done by modelling the rate of deaths rather than the raw count. The population is then added into the model as an [offset term]{.highlight}.
...
The count would need to be multiplied by $\frac{10,000}{population}$. The offset is the inverse of this which is <br> 1 $\div \frac{10,000}{population}$ =  $\frac{population}{10,000}$. As the outcome is modelled on the natural log scale, this offset will also need to be transformed. This will be added to the `glm` function when fitting a model using the `offset` argument:
...
$ln$(number of deaths) = `{r} round(coef(glm_mort_rate)[1], 2)` + `{r} round(coef(glm_mort_rate)[2], 2)` $\times$ poverty + $ln \left( \frac{10,000}{population} \right)$
...
$ln$(number of deaths) - $ln \left( \frac{10,000}{population} \right)$ = `{r} round(coef(glm_mort_rate)[1], 2)` + `{r} round(coef(glm_mort_rate)[2], 2)` $\times$ poverty

Using the natural log properties introduced earlier^[$ln(a) - ln(b) = ln(a \div b)$], the left side of the equation becomes the natural log of the mortality rate per 10,000:

$ln \left( number of deaths \div \frac{10,000}{population} \right)$ = $ln$ (mortality rate per 10,000) = `{r} round(coef(glm_mort_rate)[1], 2)` + `{r} round(coef(glm_mort_rate)[2], 2)` $\times$ poverty
...
mortality rate = $exp$(`{r} round(coef(glm_mort_rate)[1], 2)` + `{r} round(coef(glm_mort_rate)[2], 2)` $\times$ poverty) = $exp$(`{r} round(coef(glm_mort_rate)[1], 2)`) $\times exp$ (`{r} round(coef(glm_mort_rate)[2], 2)`) $^{poverty}$ = `{r} round(exp(coef(glm_mort_rate)[1]), 2)` $\times$ `{r} round(exp(coef(glm_mort_rate)[2]), 2)`$^{poverty}$

Although this model uses the same data as the first, the model now shows a positive association between poverty levels and mortality rate (as poverty increases by a percentage point, the mortality rate is expected to increase `{r} round(exp(coef(glm_mort_rate)[2]), 4)` - 1 $\times$ 100% = `{r} (round(exp(coef(glm_mort_rate)[2]), 4) - 1) * 100` %). This indicates that the previous model, using count as the outcome, was modelling differences in population rather than cancer deaths. 
